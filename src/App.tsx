...
export default function App() {
  ...
  // Lo dictado desde el widget se convierte en una acción pendiente: la misma
  // tarjeta de confirmación que usa el escaneo de recibos, donde se puede
  // corregir el tipo, el importe o la categoría antes de guardar.
  //
  // Llega por tres vías porque ninguna cubre todos los casos. En la web, por la
  // URL. En la app arrancada de cero, preguntándole al puente: el WebView carga
  // siempre la dirección de la configuración, así que un `?dictado=` en el
  // Intent no llegaría nunca. Y con la app ya abierta, como evento, porque
  // MainActivity es `singleTask` y no vuelve a montar nada.
  useEffect(() => {
    if (!user) return;

    const procesar = (crudo: string) => {
      try {
        const dictado = JSON.parse(crudo);
        setActiveTab('chat');
    ...
    if (parametro) {
      ...
      procesar(decodeURIComponent(parametro));
    }

    // El puente lo devuelve una sola vez: lo borra al entregarlo.
    try {
      const pendiente = (window as any).HeraNativo?.dictadoPendiente?.();
      if (pendiente) procesar(pendiente);
    } catch { /* Sin puente nativo no hay nada que recoger. */ }

    const alDictar = (e: Event) => procesar((e as CustomEvent).detail);
    window.addEventListener('hera:dictado', alDictar);
    return () => window.removeEventListener('hera:dictado', alDictar);
  }, [user, accounts]);
